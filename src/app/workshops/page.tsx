...
import { getWorkshops, type Workshop } from '@/lib/queries'
...
function WorkshopCard({ workshop }: { workshop: Workshop }) {
  return (
    <article className="flex flex-col sm:flex-row gap-0 bg-eco-paper border border-eco-border rounded-2xl overflow-hidden">
      {/* Data lateral */}
      {workshop.data && (
        <div className="sm:w-32 bg-eco-wood/10 flex flex-col items-center justify-center p-6 sm:p-4 text-center gap-1 flex-shrink-0">
          <span className="font-mono text-display leading-none text-eco-wood font-bold">
            {new Date(workshop.data).getDate().toString().padStart(2, '0')}
          </span>
          <span className="font-mono text-label uppercase tracking-widest text-eco-wood/70">
      ...
      <div className="flex flex-col gap-4 p-6 lg:p-8 flex-1">
        <div className="flex-1 flex flex-col gap-3">
          <h2 className="font-serif text-title text-eco-charcoal">{workshop.titulo}</h2>

          {workshop.descricao && (
            <p className="font-sans text-body text-eco-muted">{workshop.descricao}</p>
          )}

          <div className="flex flex-wrap gap-4 mt-1">
            {workshop.data && (
              <div className="flex items-center gap-2 text-eco-muted">
            ...
            {workshop.preco && (
              <div className="flex items-center gap-2 text-eco-wood">
                <svg aria-hidden="true" className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
    ...
    <PageLayout>
      ...
      <section className="bg-eco-charcoal py-section-sm">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <SectionLabel className="text-eco-wood">Encontros práticos</SectionLabel>
      ...
      <section className="bg-eco-paper border-t border-eco-border py-section-sm">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="font-serif text-title text-eco-charcoal">Quer um workshop personalizado?</h2>
